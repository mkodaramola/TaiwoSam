...
// ---------- Relays (from schematic: "Relays" net = GPIO5, drives Q4 and Q5) ----------
#define RELAY_PIN      4
#define RELAY_ACTIVE   HIGH   // NPN driver: HIGH = relays energised
#define RELAY_ON_W     30.0f  // relays turn ON above this power
#define RELAY_OFF_W    28.0f  // relays turn OFF below this power (hysteresis stops chattering)
...
void updateLcd() {
  ...
  switch (page) {
    ...
    case 1:
      ...
      lcdLine(1, "PF: " + String(data.pf, 2) + "  R:" + (relayOn ? "ON" : "OFF"));
      break;
    case 2:
...
// ---------- Relay control ----------
// ON when power > RELAY_ON_W, OFF when power < RELAY_OFF_W, unchanged in between.
// Fail-safe: if the PZEM isn't giving valid readings, relays are switched OFF.
void updateRelays() {
  bool want = relayOn;

  if (!valid)                          want = false;
  else if (data.power > RELAY_ON_W)    want = true;
  else if (data.power < RELAY_OFF_W)   want = false;

  if (want != relayOn) {
    relayOn = want;
    digitalWrite(RELAY_PIN, relayOn ? RELAY_ACTIVE : !RELAY_ACTIVE);
    Serial.printf("Relays %s (power %.1f W)\n", relayOn ? "ON" : "OFF", data.power);
  }
}

// ---------- Setup / loop ----------
void setup() {
  // Relays start OFF before anything else runs
