...
byte noc = 0;
unsigned int pMillis = 0;
unsigned int ** arr = new int*[noc+1];
unsigned int duration=0;
boolean Rec = false;
boolean t = false;
byte trigger = 3;

int ** addElem(int ** arr2,int n1, int x1,int x2,int x3,int x4){
  ...
  for(int i=0;i<n1+1;i++){
    ...
    for(int i=0;i<n1;i++){
      for(int j=0;j<4;j++) p[i][j] = arr2[i][j];
  }
...
boolean onChange(){
  ...
  if(s1 != inits1 || s2 != inits2 || s3 != inits3){
      ...
      if(noc == 0) {
        ...
        arr = addElem(arr,noc,inits1,inits2,inits3,duration);
     
     pMillis = millis();
      
      inits1 = s1; inits2 = s2; inits3 = s3;
      Serial.print("noc = "); Serial.println(noc);
      noc++;
      return true;
    }
